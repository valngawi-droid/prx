...
<section class="max-w-3xl mx-auto">
    <header class="flex items-center justify-between flex-wrap gap-3 mb-6" data-anim="fade-up">
        ...
        <div class="flex items-center gap-2">
            <form method="post" action="/pesan/baca-semua" title="Tandai semua percakapan sebagai dibaca">
                <?= csrf_field() ?>
                <button class="px-3.5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-300 text-xs hover:bg-white/10 transition">✓✓ Baca Semua</button>
            </form>
    ...
    <!-- ⭐ Pesan Tersimpan (chat dengan diri sendiri — ala Telegram Saved Messages) -->
    <a href="/pesan/<?= e(rawurlencode((string) auth_user()['username'])) ?>" class="glass-card flex items-center gap-3 p-4 mb-4 border-amber-400/25 hover:bg-white/[0.04] transition" data-anim="fade-up">
        <span class="shrink-0 w-11 h-11 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 grid place-items-center text-slate-900 text-lg">⭐</span>
        <span class="min-w-0 flex-1">
            <b class="text-sm text-white">Pesan Tersimpan</b>
            <span class="block text-xs text-slate-500">Simpan catatan, link & foto untuk dirimu sendiri — privat total 🔒</span>
        </span>
